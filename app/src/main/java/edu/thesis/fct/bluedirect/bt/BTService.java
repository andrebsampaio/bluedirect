...
public class BTService {
     ...
     */
    public BTService(Context context, ConcurrentLinkedQueue<Packet> packetQueue) {
    ...
    private class ConnectedThread extends Thread {
        ...
        public void run() {
            ...
            while (mState == STATE_CONNECTED) {
                try {
                    if (bridge == null){
                        int mInt = randInt(0,1000);
                        mmOutStream.writeInt(mInt);
                        int rInt = mmInStream.readInt();
                        if (mInt > rInt){
                            syncGroupServer(mmSocket,mmInStream,mmOutStream);
                        } else {
                            syncGroupClient(mmSocket,mmInStream,mmOutStream, MeshNetworkManager.getSelf().getGroupID());
                        }
                        mmOutStream.flush();

                    }else {
                        listenPackets(mmInStream,mmOutStream);
                    }
                } catch (IOException e) {
    ...
    private void listenPackets(DataInputStream input, DataOutputStream output) throws IOException {

        int packetSize = input.readInt();

        byte trimmedBytes[] = new byte[packetSize];
        input.readFully(trimmedBytes);
        Packet p = Packet.deserialize(trimmedBytes);
        p.setSenderIP(p.getSenderIP());
        packetQueue.add(p);
    }

    private void syncGroupServer(BluetoothSocket mSocket, DataInputStream input, DataOutputStream output) throws IOException, SameGroupException {
    ...
    private void syncGroupClient(BluetoothSocket mSocket, DataInputStream input, DataOutputStream output, String GID) throws IOException {
        output.writeUTF(GID);
        boolean ACK = input.readBoolean();
        boolean established = input.readBoolean();
        if (established) {
            String otherGID = input.readUTF();
            bridge = new Bridge(otherGID, mSocket.getRemoteDevice().getAddress());
            AllEncompasingP2PClient self = MeshNetworkManager.getSelf();
            self.setBridge(bridge);
            self.setLastUpdate(AllEncompasingP2PClient.getUnixCurrentTime());
            MeshNetworkManager.setSelf(self);
